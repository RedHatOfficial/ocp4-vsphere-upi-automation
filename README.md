...
2. Edit `group_vars/all.yml`, the following must be changed while the rest can remain the same
   ...
   * enable/disable registry/proxy and their details, as required
...
5. Ansible (preferably latest) on the machine where this repo is cloned. Before you install Ansible, install the `epel-release`, run `yum -y install epel-release` 
...
2. Get the vCenter details:
   ...
   7. Datastore name
   8. Absolute path of the vCenter folder to use *(optional)*. If this field is not populated, its is autogenerated and points to `/${vcenter.datacenter}/vm/${config.cluster_name}`
